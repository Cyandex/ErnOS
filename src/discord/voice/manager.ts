...
function resolveVoiceTtsConfig(params: { cfg: ErnOSConfig; override?: TtsConfig }): {
  cfg: ErnOSConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredErnOSTmpDir(), "discord-voice-"));
