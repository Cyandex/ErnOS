...
const TEMP_MEDIA_PREFIX = "ernos-media-";
...
function createGroqAudioConfig(): ErnOSConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ErnOSConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ErnOSConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ErnOSConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredErnOSTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ERNOS_AGENT_DIR: isolatedAgentDir,
