...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ERNOS_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ERNOS_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ERNOS_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ERNOS_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
