...
import { resolveStateDir, type ErnOSConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: ErnOSConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.ERNOS_AUTH_STORE_READONLY;
  process.env.ERNOS_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as ErnOSConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.ERNOS_AUTH_STORE_READONLY;
    } else {
      process.env.ERNOS_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
