...
function getSecretProviders(config: ErnOSConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: ErnOSConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: ErnOSConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: ErnOSConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: ErnOSConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: ErnOSConfig; next: ErnOSConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in ernos.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "ernos secrets configure",
