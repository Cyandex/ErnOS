...
function resolveGatewayPort(cfg: ErnOSConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: ErnOSConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.ERNOS_GATEWAY_PASSWORD?.trim() ||
    env.ERNOS_GATEWAY_PASSWORD?.trim() ||
