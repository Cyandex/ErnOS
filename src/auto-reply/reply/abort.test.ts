...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop ernos!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@ernos_bot", { botUsername: "ernos_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@ernos_bot", { botUsername: "ernos_bot" })).toBe(true);
