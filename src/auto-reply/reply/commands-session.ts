...
import { scheduleGatewaySigusr1Restart, triggerErnOSRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ErnOS in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerErnOSRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ErnOS via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
