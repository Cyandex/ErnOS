...
export function resolveConfiguredAcpBackendId(cfg: ErnOSConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: ErnOSConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `ernos plugins install ${localPath}`;
    }
    return "ernos plugins install @ernos/acpx";
  }
