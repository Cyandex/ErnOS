...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "ernos-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ERNOS_GATEWAY_TOKEN: "test-token",
    ERNOS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ERNOS_WORKSPACE_DIR: join(sandbox.rootDir, "ernos"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ERNOS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ERNOS_EXTRA_MOUNTS: undefined,
      ERNOS_HOME_VOLUME: "ernos-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ERNOS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ERNOS_EXTRA_MOUNTS=");
    expect(envFile).toContain("ERNOS_HOME_VOLUME=ernos-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("ernos-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("ernos-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ERNOS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ERNOS_CONFIG_DIR: configDir,
      ERNOS_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when ERNOS_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ERNOS_GATEWAY_TOKEN=config-token-123");
  });

  it("rejects injected multiline ERNOS_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ERNOS_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ERNOS_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ERNOS_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ERNOS_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ERNOS_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ERNOS_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ERNOS_HOME_VOLUME must match");
  });
