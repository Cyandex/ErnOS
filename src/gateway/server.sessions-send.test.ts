...
const { createErnOSTools } = await import("../agents/ernos-tools.js");
...
type SessionSendTool = ReturnType<typeof createErnOSTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createErnOSTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ERNOS_GATEWAY_PORT", "ERNOS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ERNOS_GATEWAY_PORT = String(gatewayPort);
  process.env.ERNOS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ERNOS_CONFIG_PATH missing in gateway test environment");
      }
