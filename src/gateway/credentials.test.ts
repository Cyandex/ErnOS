...
function cfg(input: Partial<ErnOSConfig>): ErnOSConfig {
  return input as ErnOSConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<ErnOSConfig["gateway"]>;
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy ERNOS env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
    ...
    expect(resolved).toEqual({ token: "legacy-token", password: "legacy-password" });
  });
});
