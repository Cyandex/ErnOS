...
import { createErnOSTools } from "../agents/ernos-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-ernos-message-channel") ?? "");
  const accountId = getHeader(req, "x-ernos-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-ernos-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-ernos-thread-id")?.trim() || undefined;
  ...
  const allTools = createErnOSTools({
