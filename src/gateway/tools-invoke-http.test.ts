...
let lastCreateErnOSToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/ernos-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateErnOSToolsContext?.agentTo,
          agentThreadId: lastCreateErnOSToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createErnOSTools: (ctx: Record<string, unknown>) => {
      lastCreateErnOSToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateErnOSToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-ernos-message-to": "channel:24514",
        "x-ernos-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
