...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ERNOS_STATE_DIR",
  "ERNOS_CONFIG_PATH",
  "ERNOS_SKIP_BROWSER_CONTROL_SERVER",
  "ERNOS_SKIP_GMAIL_WATCHER",
  "ERNOS_SKIP_CANVAS_HOST",
  "ERNOS_BUNDLED_PLUGINS_DIR",
  "ERNOS_SKIP_CHANNELS",
  "ERNOS_SKIP_PROVIDERS",
  "ERNOS_SKIP_CRON",
  "ERNOS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ERNOS_STATE_DIR = path.join(tempHome, ".ernos");
  delete process.env.ERNOS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ERNOS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ERNOS_SKIP_GMAIL_WATCHER = "1";
  process.env.ERNOS_SKIP_CANVAS_HOST = "1";
  process.env.ERNOS_SKIP_CHANNELS = "1";
  process.env.ERNOS_SKIP_PROVIDERS = "1";
  process.env.ERNOS_SKIP_CRON = "1";
  process.env.ERNOS_TEST_MINIMAL_GATEWAY = "1";
  process.env.ERNOS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "ernos-test-no-bundled-extensions")
    : "ernos-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "ernos-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".ernos-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__ernosTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__ernosTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ERNOS_GATEWAY_TOKEN"]);
  const prev = process.env.ERNOS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ERNOS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.ERNOS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
