...
export function resolveHooksConfig(cfg: ErnOSConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ErnOSConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-ernos-token"] === "string" ? req.headers["x-ernos-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
