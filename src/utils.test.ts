...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("ernos-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("ernos-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("ernos-lid-a-", (first) => {
      withTempDirSync("ernos-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.ernos when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ernos-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ERNOS_HOME over HOME", () => {
    vi.stubEnv("ERNOS_HOME", "/srv/ernos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/ernos-home"));
...
describe("shortenHomePath", () => {
  it("uses $ERNOS_HOME prefix when ERNOS_HOME is set", () => {
    vi.stubEnv("ERNOS_HOME", "/srv/ernos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/ernos-home")}/.ernos/ernos.json`)).toBe(
      "$ERNOS_HOME/.ernos/ernos.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ERNOS_HOME replacement when ERNOS_HOME is set", () => {
    vi.stubEnv("ERNOS_HOME", "/srv/ernos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/ernos-home")}/.ernos/ernos.json`),
    ).toBe("config: $ERNOS_HOME/.ernos/ernos.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/ernos")).toBe(path.resolve(os.homedir(), "ernos"));
  });
  ...
  it("prefers ERNOS_HOME for tilde expansion", () => {
    vi.stubEnv("ERNOS_HOME", "/srv/ernos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/ernos")).toBe(path.resolve("/srv/ernos-home", "ernos"));
