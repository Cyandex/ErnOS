...
function cfg(accounts?: Record<string, unknown> | null): ErnOSConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as ErnOSConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as ErnOSConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as ErnOSConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as ErnOSConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as ErnOSConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as ErnOSConfig)).toBe("default");
    });
  });
});
