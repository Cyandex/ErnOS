...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: ErnOSConfig, policy: ChannelAccessPolicy) => ErnOSConfig;
  resolveAllowlist: (params: { cfg: ErnOSConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: ErnOSConfig; resolved: TResolved }) => ErnOSConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: ErnOSConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: ErnOSConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: ErnOSConfig, policy: ChannelAccessPolicy): ErnOSConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: ErnOSConfig, policy: ChannelAccessPolicy): ErnOSConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: ErnOSConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: ErnOSConfig; resolved: string[] }) => {
      calls.push("apply");
