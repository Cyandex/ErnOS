...
function telegramCfg(): ErnOSConfig {
  return { channels: { telegram: { botToken: "tok" } } } as ErnOSConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: ErnOSConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as ErnOSConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as ErnOSConfig;
...
function createSignalAccountOverrideCfg(): ErnOSConfig {
  return {
    channels: {
      signal: {
...
function createDiscordModerationOverrideCfg(params?: { channelsEnabled?: boolean }): ErnOSConfig {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as ErnOSConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as ErnOSConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as ErnOSConfig,
        accountId: undefined,
        params: {
