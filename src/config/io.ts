...
import type { ErnOSConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareErnOSVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  ernos config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  ernos config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: ErnOSConfig,
  pathSegments: string[],
): { changed: boolean; next: ErnOSConfig } {
...
function coerceConfig(value: unknown): ErnOSConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ErnOSConfig;
}
...
function stampConfigVersion(cfg: ErnOSConfig): ErnOSConfig {
...
function warnIfConfigFromFuture(cfg: ErnOSConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareErnOSVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ErnOS (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ErnOSConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ErnOSConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ErnOSConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ErnOSConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ErnOSConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ERNOS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ERNOS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ERNOS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ERNOS_WATCH_SESSION === "string" &&
        deps.env.ERNOS_WATCH_SESSION.trim().length > 0
          ? deps.env.ERNOS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ERNOS_WATCH_COMMAND === "string" &&
        deps.env.ERNOS_WATCH_COMMAND.trim().length > 0
          ? deps.env.ERNOS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ERNOS_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ErnOSConfig | null = null;
let runtimeConfigSourceSnapshot: ErnOSConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ERNOS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ERNOS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(config: ErnOSConfig, sourceConfig?: ErnOSConfig): void {
...
export function getRuntimeConfigSnapshot(): ErnOSConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): ErnOSConfig {
