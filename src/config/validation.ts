...
import type { ErnOSConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ErnOSConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ErnOSConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ErnOSSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ErnOSConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ErnOSConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as ErnOSConfig,
  };
}
