...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("ernos-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "ernos-sandbox-browser-global" },
      agentBrowser: { network: "ernos-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("ernos-sandbox-browser-agent");
  });
