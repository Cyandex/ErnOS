...
import type { ConfigFileSnapshot } from "./types.ernos.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.ernos/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/ernos/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/ernos/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/ernos/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/ernos/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(ErnOSSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ErnOSSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ErnOSConfig";
    const hints = mapSensitivePaths(ErnOSSchema, "", {});
