...
// Prefix for ErnOS-generated action IDs to scope our handler
const ERNOS_ACTION_PREFIX = "ernos:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  // Handle Block Kit button clicks from ErnOS-generated messages
  // Only matches action_ids that start with our prefix to avoid interfering
  // with other Slack integrations or future features
  ctx.app.action(new RegExp(`^${ERNOS_ACTION_PREFIX}`), async (args: SlackActionMiddlewareArgs) => {
  ...
  const modalMatcher = new RegExp(`^${ERNOS_ACTION_PREFIX}`);

  // Handle ErnOS modal submissions with callback_ids scoped by our prefix.
