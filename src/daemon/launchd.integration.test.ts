...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ernos-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ERNOS_LAUNCHD_LABEL: `ai.ernos.launchd-int-${testId}`,
      ERNOS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
