...
function listAgents(cfg: ErnOSConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ErnOSConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ErnOSConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ErnOSConfig, EvaluatedBindingsCache>();
