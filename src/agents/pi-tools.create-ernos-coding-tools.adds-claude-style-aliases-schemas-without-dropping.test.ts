...
import { __testing, createErnOSCodingTools } from "./pi-tools.js";
import { createErnOSReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { isOwnerOnlyToolName } from "./tool-policy.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createErnOSCodingTools({ senderIsOwner: true });
...
describe("createErnOSCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createErnOSCodingTools({ senderIsOwner: true }).find(
  ...
  it("avoids anyOf/oneOf/allOf in tool schemas", () => {
    // Owner-only tools (e.g. devteam) are excluded — they only run for owners
    // and are not subject to third-party model schema compatibility constraints.
    const publicTools = defaultTools.filter((t) => !isOwnerOnlyToolName(t.name));
    expect(findUnionKeywordOffenders(publicTools)).toEqual([]);
  });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createErnOSCodingTools({ senderIsOwner: true, messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createErnOSCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "ernos-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createErnOSReadTool(
      baseRead as unknown as Parameters<typeof createErnOSReadTool>[0],
    );
