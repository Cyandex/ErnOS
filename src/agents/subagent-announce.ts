...
const FAST_TEST_MODE = process.env.ERNOS_TEST_FAST === "1";
const FAST_TEST_RETRY_INTERVAL_MS = 8;
const FAST_TEST_REPLY_CHANGE_WAIT_MS = 20;
const DEFAULT_SUBAGENT_ANNOUNCE_TIMEOUT_MS = 15 * 60_000;
    ...
    const result = await hookRunner.runSubagentDeliveryTarget(
      {
        ...
        spawnMode: params.spawnMode as "run" | "session" | undefined,
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to ErnOS sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`ernos ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for ErnOS subagents (`runtime: "subagent"`).',
