...
describe("createErnOSCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: ErnOSConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createErnOSCodingTools({
      senderIsOwner: true,
    ...
    const anthropicTools = createErnOSCodingTools({
      senderIsOwner: true,
      config: enabledConfig,
    ...
    const allowModelsConfig: ErnOSConfig = {
      tools: {
        exec: {
    ...
    const allowed = createErnOSCodingTools({
      senderIsOwner: true,
    ...
    const denied = createErnOSCodingTools({
      senderIsOwner: true,
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    ...
    const oauthTools = createErnOSCodingTools({
      senderIsOwner: true,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    ...
    // Tape tools use null/undefined parameters (they are schemaless by design)
    const schemalessTools = new Set([
      "tape_seek",
      "tape_read",
      "tape_write",
      "tape_scan",
    ...
    const offenders = tools
      .filter((tool) => !schemalessTools.has(tool.name))
      .map((tool) => {
        const schema =
