...
describe("createErnOSCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-read-"));
    try {
      ...
      const contents = "Hello from ernos read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "ernos-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "ernos-workspace"),
    ...
    const tools = createErnOSCodingTools({ senderIsOwner: true, sandbox });
