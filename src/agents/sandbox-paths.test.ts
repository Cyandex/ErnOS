...
async function withOutsideHardlinkInErnOSTmp<T>(
  params: {
    ernOSTmpDir: string;
  ...
  const hardlinkPath = path.join(params.ernOSTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.ernOSTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.ernOSTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.ernOSTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const ernOSTmpDir = resolvePreferredErnOSTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred ErnOS tmp root",
      media: path.join(ernOSTmpDir, "image.png"),
      expected: path.join(ernOSTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred ErnOS tmp root",
      media: pathToFileURL(path.join(ernOSTmpDir, "photo.png")).href,
      expected: path.join(ernOSTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred ErnOS tmp root",
      media: path.join(ernOSTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(ernOSTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(ernOSTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside ernos tmp root",
      media: path.join(os.tmpdir(), "outside-ernos", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked ErnOS tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(ernOSTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(ernOSTmpDir, { recursive: true });
      const symlinkPath = path.join(ernOSTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked ErnOS tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInErnOSTmp(
      {
        ernOSTmpDir,
  ...
  it("rejects symlinked ErnOS tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInErnOSTmp(
