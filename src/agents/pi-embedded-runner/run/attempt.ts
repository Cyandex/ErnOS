...
import { createErnOSCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const isDmSession = isSubagentSessionKey(params.sessionKey || "");
    const contextObj = await systemMemory.recall(
      params.prompt,
      params.senderId || "Unknown",
      undefined,
      isDmSession,
      params.senderName || "Unknown",
    );
    let memoryStoreContext = `---
## Five-Tier Cognitive Memory

### Working Memory
${contextObj.workingMemory || "No recent working memory."}

### Related Vector Memories
${contextObj.relatedMemories?.join("\\n") || "No vector memories found."}

### Knowledge Graph Context
${contextObj.knowledgeGraph || "No known semantic relations."}

### Crystallized Lessons
${contextObj.lessons?.map((l: string) => "- " + l).join("\\n") || "No crystallized lessons."}

### Tape Machine State (Internal Monologue)
${contextObj.tapeView || "No tape history."}
---`;
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/ernos/ernos/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("ernos:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/ernos/ernos/issues/8643
