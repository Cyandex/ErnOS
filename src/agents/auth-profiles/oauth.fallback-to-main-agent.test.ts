...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["ERNOS_STATE_DIR", "ERNOS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveErnOSAgentDir() returns mainAgentDir
    process.env.ERNOS_STATE_DIR = tmpDir;
    process.env.ERNOS_AGENT_DIR = mainAgentDir;
