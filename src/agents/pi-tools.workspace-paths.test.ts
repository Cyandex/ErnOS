...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("ernos-ws-", async (workspaceDir) => {
      await withTempDir("ernos-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createErnOSCodingTools({ senderIsOwner: true, workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello ernos");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("ernos-ws-", async (workspaceDir) => {
      await withTempDir("ernos-override-", async (overrideDir) => {
        const tools = createErnOSCodingTools({
          senderIsOwner: true,
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("ernos-ws-", async (workspaceDir) => {
      const cfg: ErnOSConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createErnOSCodingTools({ senderIsOwner: true, workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-ernos.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("ernos-sandbox-", async (sandboxDir) => {
      await withTempDir("ernos-workspace-", async (workspaceDir) => {
        ...
        const tools = createErnOSCodingTools({ senderIsOwner: true, workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
