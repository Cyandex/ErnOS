...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    ernos: process.env.ERNOS_GATEWAY_TOKEN,
    ernosLegacy: process.env.ERNOS_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.ernos === undefined) {
      delete process.env.ERNOS_GATEWAY_TOKEN;
    } else {
      process.env.ERNOS_GATEWAY_TOKEN = envSnapshot.ernos;
    }
    if (envSnapshot.ernos === undefined) {
      delete process.env.ERNOS_GATEWAY_TOKEN;
    } else {
  ...
  it("uses ERNOS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ERNOS_GATEWAY_TOKEN = "local-env-token";
    process.env.ERNOS_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
