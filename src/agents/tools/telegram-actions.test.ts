...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ErnOSConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ErnOSConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ErnOSConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ErnOSConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: ErnOSConfig, accountId = "media") {
    await handleTelegramAction(
