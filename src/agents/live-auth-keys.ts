...
const GOOGLE_LIVE_SINGLE_KEY = "ERNOS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ERNOS_LIVE_ANTHROPIC_KEY",
    listVar: "ERNOS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ERNOS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ERNOS_LIVE_${base}_KEY`;
