...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const ernOSTmpDir = path.resolve(resolvePreferredErnOSTmpDir());
  if (!isPathInside(ernOSTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: ernOSTmpDir });
  return resolved;
}
