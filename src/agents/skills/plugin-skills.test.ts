    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "ernos.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "ernos.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("ernos-");
    const acpxRoot = await tempDirs.make("ernos-acpx-plugin-");
    const helperRoot = await tempDirs.make("ernos-helper-plugin-");
