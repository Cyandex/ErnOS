...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ERNOS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".ernos", `workspace-${profile}`);
  }
  return path.join(home, ".ernos", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".ernos";
