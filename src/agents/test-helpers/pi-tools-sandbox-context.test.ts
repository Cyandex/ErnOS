...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("ernos-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("ernos-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("ernos-sbx-");
