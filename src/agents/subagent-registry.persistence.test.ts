...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ERNOS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-subagent-"));
  ...
  it("uses isolated temp state when ERNOS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "ernos-test-state"));
  });
});
