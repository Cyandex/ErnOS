...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ErnOSConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ErnOSConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ErnOSConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: ErnOSConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ErnOSConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: ErnOSConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/ernos");
  });

  it("uses ERNOS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "ernos-home");
    vi.stubEnv("ERNOS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as ErnOSConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".ernos", "workspace"));
  });

  it("uses ERNOS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "ernos-home");
    vi.stubEnv("ERNOS_HOME", home);
    // Clear state dir so it falls back to ERNOS_HOME
    vi.stubEnv("ERNOS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as ErnOSConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".ernos", "agents", "main", "agent"));
  });
});
