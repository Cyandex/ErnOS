...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "ernos.sandbox=1");
  args.push("--label", `ernos.sessionKey=${params.scopeKey}`);
  args.push("--label", `ernos.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `ernos.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "ernos.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`ernos sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`ernos sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("ernos sandbox recreate --all");
}
