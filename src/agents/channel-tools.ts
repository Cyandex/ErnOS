...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as ErnOSConfig);
...
export function listChannelAgentTools(params: { cfg?: ErnOSConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: ErnOSConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
