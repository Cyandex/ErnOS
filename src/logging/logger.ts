...
export const DEFAULT_LOG_DIR = resolvePreferredErnOSTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "ernos.log"); // legacy single-file path

const LOG_PREFIX = "ernos";
...
function resolveSettings(): ResolvedSettings {
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ERNOS_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[ernos] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
