...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "ernos.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "ernos.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ERNOS_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ERNOS_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ERNOS_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[ernos] ${parsed.error}`);
      process.exit(2);
    }
