...
const RELAY_AUTH_HEADER = "x-ernos-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/ErnOS-Extension-Relay",
            revision: "0",
            userAgent: "ErnOS-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedErnOSRelay({
