...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and sprout-orange color", () => {
    ...
    const ernos = resolveProfile(resolved, "ernos");
    expect(ernos?.driver).toBe("ernos");
    expect(ernos?.cdpPort).toBe(18800);
    expect(ernos?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from ERNOS_GATEWAY_PORT when unset", () => {
    withEnv({ ERNOS_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const ernos = resolveProfile(resolved, "ernos");
      expect(ernos?.cdpPort).toBe(19012);
      expect(ernos?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ERNOS_GATEWAY_PORT: undefined }, () => {
      ...
      const ernos = resolveProfile(resolved, "ernos");
      expect(ernos?.cdpPort).toBe(19022);
      expect(ernos?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "ernos");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        ernos: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("ernos");
  });
