import type { BrowserConfig, BrowserProfileConfig, ErnOSConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "ernos" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ERNOS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "ernos" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ERNOS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ERNOS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ErnOS browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for ernos/ernos CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ErnOSConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ERNOS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "ernos";
