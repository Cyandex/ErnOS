...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".ernos-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + sprout-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateErnOSProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ERNOS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ERNOS_BROWSER_COLOR).toUpperCase();
