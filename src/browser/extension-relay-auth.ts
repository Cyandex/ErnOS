...
const RELAY_TOKEN_CONTEXT = "ernos-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ERNOS_RELAY_BROWSER = "ErnOS/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ERNOS_GATEWAY_TOKEN?.trim() || process.env.ERNOS_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or ERNOS_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedErnOSRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ERNOS_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
