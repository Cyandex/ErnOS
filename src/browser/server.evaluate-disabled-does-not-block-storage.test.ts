  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          ernos: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ERNOS_GATEWAY_PORT;
    process.env.ERNOS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ERNOS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ERNOS_GATEWAY_PASSWORD;
