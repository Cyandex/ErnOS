...
vi.mock("./ernos-root.js", () => ({
  resolveErnOSPackageRoot: vi.fn(async () => null),
  resolveErnOSPackageRootSync: vi.fn(() => null),
}));
...
let ernosRoot: typeof import("./ernos-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    ernosRoot = await import("./ernos-root.js");
  });
  ...
  it("uses resolveErnOSPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/ernos");
    (
      ernosRoot.resolveErnOSPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/ernos"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "ernos" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "ernos.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-ernos");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/ernos-bundle");
    (
      ernosRoot.resolveErnOSPackageRootSync as unknown as ReturnType<typeof vi.fn>
