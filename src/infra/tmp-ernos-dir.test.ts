...
import { POSIX_ERNOS_TMP_DIR, resolvePreferredErnOSTmpDir } from "./tmp-ernos-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredErnOSTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `ernos-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredErnOSTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredErnOSTmpDir", () => {
  it("prefers /tmp/ernos when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ERNOS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/ernos when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ERNOS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/ernos when /tmp/ernos is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/ernos when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/ernos is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/ernos is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/ernos is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback ErnOS temp dir/);
  });
