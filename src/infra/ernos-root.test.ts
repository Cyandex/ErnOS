...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__ernos_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "ernos-root");
...
describe("resolveErnOSPackageRoot", () => {
  let resolveErnOSPackageRoot: typeof import("./ernos-root.js").resolveErnOSPackageRoot;
  let resolveErnOSPackageRootSync: typeof import("./ernos-root.js").resolveErnOSPackageRootSync;

  beforeAll(async () => {
    ({ resolveErnOSPackageRoot, resolveErnOSPackageRootSync } = await import("./ernos-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "ernos");
    const pkgRoot = path.join(project, "node_modules", "ernos");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "ernos" }));

    expect(resolveErnOSPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "ernos");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "ernos.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "ernos" }));

    expect(resolveErnOSPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveErnOSPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-ernos package roots", async () => {
    const pkgRoot = fx("not-ernos");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-ernos" }));

    expect(resolveErnOSPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveErnOSPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveErnOSPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
