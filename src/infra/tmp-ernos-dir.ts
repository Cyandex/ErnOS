...
export const POSIX_ERNOS_TMP_DIR = "/tmp/ernos";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredErnOSTmpDirOptions = {
...
export function resolvePreferredErnOSTmpDir(
  options: ResolvePreferredErnOSTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "ernos" : `ernos-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[ernos] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback ErnOS temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback ErnOS temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ERNOS_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ERNOS_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ERNOS_TMP_DIR)) {
      return POSIX_ERNOS_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ERNOS_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ERNOS_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ERNOS_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ERNOS_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ERNOS_TMP_DIR;
