...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ErnOSConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ErnOSConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ErnOSConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: ErnOSConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: ErnOSConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ErnOSConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ErnOSConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ErnOSConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
