...
function candidateBinDirs(opts: EnsureErnOSPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `ernos` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "ernos");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ERNOS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "ernos"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `ernos` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureErnOSCliOnPath(opts: EnsureErnOSPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ERNOS_PATH_BOOTSTRAPPED)) {
    return;
  }
