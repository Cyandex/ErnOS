...
let ensureErnOSCliOnPath: typeof import("./path-env.js").ensureErnOSCliOnPath;

describe("ensureErnOSCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ERNOS_PATH_BOOTSTRAPPED",
    "ERNOS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureErnOSCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling ernos exists", () => {
    const tmp = abs("/tmp/ernos-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "ernos");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ERNOS_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.ERNOS_PATH_BOOTSTRAPPED = "1";
    ensureErnOSCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/ernos-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ernos");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/ernos-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ernos");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "ernos");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/ernos-path/case-linuxbrew");
