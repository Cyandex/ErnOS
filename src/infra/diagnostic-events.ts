...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __ernosDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__ernosDiagnosticEventsState) {
    globalStore.__ernosDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__ernosDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: ErnOSConfig): boolean {
