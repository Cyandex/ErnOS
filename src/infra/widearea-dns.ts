...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ERNOS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*ernos-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "ernos";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "ernos");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "ernos-gw");
  ...
  records.push(`_ernos-gw._tcp IN PTR ${instanceLabel}._ernos-gw._tcp`);
  records.push(`${instanceLabel}._ernos-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._ernos-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; ernos-content-hash: ${contentHash}\n${contentBody}`;
}
