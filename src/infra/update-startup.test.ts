...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ERNOS_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ERNOS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveErnOSPackageRoot } = await import("./ernos-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveErnOSPackageRoot).mockResolvedValue("/opt/ernos");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/ernos/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
