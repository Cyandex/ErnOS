...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ERNOS_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/ernos-home"));
  });
  ...
  it("expands ERNOS_HOME when set to ~", () => {
    const env = {
      ERNOS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ERNOS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ERNOS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ERNOS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ERNOS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ERNOS_HOME: "/srv/ernos-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/ernos-home")}/x`);
  });
