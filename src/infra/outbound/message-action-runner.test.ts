...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred ErnOS tmp root", async () => {
    const tmpRoot = resolvePreferredErnOSTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideErnOS = path.join(os.tmpdir(), "outside-ernos", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
