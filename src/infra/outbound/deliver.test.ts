...
import { resolvePreferredErnOSTmpDir } from "../tmp-ernos-dir.js";
...
const telegramChunkConfig: ErnOSConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: ErnOSConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes ErnOS tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredErnOSTmpDir()]),
      }),
    );
  });

  it("includes ErnOS tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes ErnOS tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes ErnOS tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: ErnOSConfig = { channels: { signal: { mediaMaxMb: 2 } } };
