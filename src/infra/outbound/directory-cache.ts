...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ErnOSConfig | null = null;
  ...
  get(key: string, cfg: ErnOSConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ErnOSConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ErnOSConfig): void {
