...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ERNOS_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "ErnOS";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ErnOS\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ERNOS_MDNS_HOSTNAME?.trim() || process.env.ERNOS_MDNS_HOSTNAME?.trim() || "ernos";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
