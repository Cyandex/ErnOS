...
const DEFAULT_PACKAGE_NAME = "ernos";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "ernos.mjs");
    ...
    const doctorStep = await runStep(
      step("ernos doctor", doctorArgv, gitRoot, { ERNOS_UPDATE_IN_PROGRESS: "1" }),
    );
