...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ERNOS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ERNOS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ERNOS_APNS_TEAM_ID and ERNOS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ERNOS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ERNOS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ERNOS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ERNOS_APNS_PRIVATE_KEY_P8 or ERNOS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ERNOS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createErnOSPushMetadata(params: {
