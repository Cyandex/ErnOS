...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): ErnOSConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-mem-read-"));
