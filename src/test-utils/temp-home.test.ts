...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("ernos-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.ERNOS_STATE_DIR).toBe(path.join(tempHome.home, ".ernos"));
    ...
    expect(process.env.ERNOS_STATE_DIR).toBe(previousStateDir);
