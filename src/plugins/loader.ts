...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ErnOSPluginDefinition;
  register?: ErnOSPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ErnOSPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ErnOSPluginDefinition;
...
export function loadErnOSPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverErnOSPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "ernos/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "ernos/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ErnOSPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as ErnOSPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
