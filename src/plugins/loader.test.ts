...
import { __testing, loadErnOSPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `ernos-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ERNOS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "ernos.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.ERNOS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadErnOSPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadErnOSPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ERNOS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ERNOS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadErnOSPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadErnOSPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadErnOSPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.ERNOS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ ERNOS_STATE_DIR: stateDir }, () => {
