...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ernos-doctor-"));
  tempRoot = root;
  return root;
}
...
async function detectAndRunMigrations(params: {
  ...
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { ERNOS_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".ernos"),
    legacyDir: path.join(root, ".ernos"),
  };
}
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      cfg,
      env: { ERNOS_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: ErnOSConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
  ...
  it("does not warn when legacy state dir is the same as target dir (post-rebrand)", async () => {
    const root = await makeTempRoot();
    ...
    // After rebrand, legacyDir === targetDir, so no migration needed
    const result = await runStateDirMigration(root);
  ...
  it("does not warn when legacy and target dirs are identical (post-rebrand nested check)", async () => {
    const root = await makeTempRoot();
    const { targetDir } = ensureLegacyAndTargetStateDirs(root);
    fs.mkdirSync(path.join(targetDir, "agents", "main"), { recursive: true });

    // After rebrand, legacyDir === targetDir, so no nested symlink migration needed
    const result = await runStateDirMigration(root);

    expect(result.migrated).toBe(false);
  });

  it("handles legacy=target when checking symlink targets (post-rebrand)", async () => {
    const root = await makeTempRoot();
    const { targetDir } = ensureLegacyAndTargetStateDirs(root);
    fs.mkdirSync(path.join(targetDir, "sessions"), { recursive: true });
  ...
  it("handles broken target gracefully when legacy=target (post-rebrand)", async () => {
    const root = await makeTempRoot();
    const { targetDir } = ensureLegacyAndTargetStateDirs(root);

    // After rebrand, legacyDir === targetDir, just verify no crash
    const result = await runStateDirMigration(root);
