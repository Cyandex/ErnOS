...
const SEARCH_TOOL = "https://docs.ernos.ai/mcp.SearchErnOS";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.ernos.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('ernos docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.ernos.ai/");
      runtime.log(`Search: ${formatCliCommand('ernos docs "your query"')}`);
    }
    return;
  }
