...
import type { ErnOSConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ErnOSConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("ernos security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.ernos/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("ernos approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("ernos configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("ernos config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("ernos doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("ernos config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('ernos config set session.dmScope "per-channel-peer"') +
