...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveErnOSAgentDir: vi.fn().mockReturnValue("/tmp/ernos-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/ernos-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/ernos-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveErnOSAgentDir: mocks.resolveErnOSAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/ernos-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveErnOSAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/ernos-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/ernos-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/ernos-agent-custom");
