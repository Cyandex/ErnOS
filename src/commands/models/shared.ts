...
import { type ErnOSConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<ErnOSConfig> {
...
export async function updateConfig(
  mutator: (cfg: ErnOSConfig) => ErnOSConfig,
): Promise<ErnOSConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: ErnOSConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ErnOSConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("ernos agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
