...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveErnOSAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("ernos models auth setup-token")}\` or \`${formatCliCommand("ernos configure")}\`.`
          : `Run \`${formatCliCommand("ernos configure")}\` or set an API key env var.`;
