...
export function normalizeCompatibilityConfigValues(cfg: ErnOSConfig): {
  config: ErnOSConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: ErnOSConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as ErnOSConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as ErnOSConfig["browser"],
    };
    changes.push(
