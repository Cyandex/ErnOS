...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ErnOS status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`ernos devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("ernos devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("ernos devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ernos security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ernos security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.ernos.ai/faq");
  runtime.log("Troubleshooting: https://docs.ernos.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ernos status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ernos logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ernos status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("ernos gateway probe")}`);
  }
}
