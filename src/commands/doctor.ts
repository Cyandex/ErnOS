...
function resolveMode(cfg: ErnOSConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveErnOSPackageRoot({
  ...
  let cfg: ErnOSConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("ernos configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("ernos config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("ernos setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("ernos doctor --fix")}" to apply changes.`);
  }
