 ...
 * Runs as part of `ernos doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("ernos config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("ernos memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("ernos configure --section model")}`,
        `- To disable: ${formatCliCommand("ernos config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("ernos memory status --deep")}`,
      ].join("\n"),
      "Memory search",
