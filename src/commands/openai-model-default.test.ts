...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ErnOSConfig },
  primary: string,
) {
...
function expectConfigUnchanged(applied: { changed: boolean; next: ErnOSConfig }, cfg: ErnOSConfig) {
  expect(applied.changed).toBe(false);
  expect(applied.next).toEqual(cfg);
}

type SharedDefaultModelCase = {
  apply: (cfg: ErnOSConfig) => { changed: boolean; next: ErnOSConfig };
  defaultModel: string;
  overrideConfig: ErnOSConfig;
  alreadyDefaultConfig: ErnOSConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ErnOSConfig) => config,
      applyDefaultConfig: (config: ErnOSConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ErnOSConfig = { agents: { defaults: {} } };
