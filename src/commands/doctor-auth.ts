...
function pruneAuthProfiles(
  cfg: ErnOSConfig,
  profileIds: Set<string>,
): { next: ErnOSConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("ernos models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("ernos models auth setup-token")} or ${formatCliCommand(
      "ernos configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "ernos models auth login --provider openai-codex",
    )} or ${formatCliCommand("ernos configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("ernos configure")}\` or \`${formatCliCommand("ernos onboard")}\`.`;
}
