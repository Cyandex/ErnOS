...
type PromptDefaultModelResult = { model?: string; config?: ErnOSConfig };
...
function resolveConfiguredModelRaw(cfg: ErnOSConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: ErnOSConfig): string[] {
...
export function applyPrimaryModel(cfg: ErnOSConfig, model: string): ErnOSConfig {
...
export function applyModelAllowlist(cfg: ErnOSConfig, models: string[]): ErnOSConfig {
  const defaults = cfg.agents?.defaults;
