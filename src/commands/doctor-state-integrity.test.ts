...
type EnvSnapshot = {
  HOME?: string;
  ERNOS_HOME?: string;
  ERNOS_STATE_DIR?: string;
  ERNOS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ERNOS_HOME: process.env.ERNOS_HOME,
    ERNOS_STATE_DIR: process.env.ERNOS_STATE_DIR,
    ERNOS_OAUTH_DIR: process.env.ERNOS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ErnOSConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ErnOSConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "ernos-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ERNOS_HOME = tempHome;
    process.env.ERNOS_STATE_DIR = path.join(tempHome, ".ernos");
    delete process.env.ERNOS_OAUTH_DIR;
    fs.mkdirSync(process.env.ERNOS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ERNOS_OAUTH_DIR is explicitly configured", async () => {
    process.env.ERNOS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ErnOSConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints ernos-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ErnOSConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/ernos sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/ernos sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /ernos sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
