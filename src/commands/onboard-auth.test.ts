...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ERNOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ernos-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-oauth-sync-"));
    ...
    process.env.ERNOS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-oauth-nosync-"));
    process.env.ERNOS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ERNOS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-oauth-external-"));
    process.env.ERNOS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ERNOS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ERNOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ernos-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
