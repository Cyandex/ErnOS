...
function stripUnknownConfigKeys(config: ErnOSConfig): {
  config: ErnOSConfig;
  removed: string[];
} {
  const parsed = ErnOSSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: ErnOSConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: ErnOSConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: ErnOSConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: ErnOSConfig): Promise<{
  config: ErnOSConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: ErnOSConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: ErnOSConfig): {
  config: ErnOSConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: ErnOSConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: ErnOSConfig): {
  config: ErnOSConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: ErnOSConfig): Promise<{
  config: ErnOSConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: ErnOSConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    warnings.push(
      `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("ernos doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
    );
  };
...
function collectExecSafeBinScopes(cfg: ErnOSConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: ErnOSConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: ErnOSConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: ErnOSConfig): {
  config: ErnOSConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: ErnOSConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: ErnOSConfig): {
  config: ErnOSConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".ernos");
  const targetPath = path.join(targetDir, "ernos.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".ernos", "ernos.json"),
    path.join(home, ".ernos", "ernos.json"),
    path.join(home, ".ernos", "ernos.json"),
  ];
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("ernos doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("ernos doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("ernos doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("ernos doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("ernos doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("ernos doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("ernos doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "ernos doctor --fix" to remove these keys.');
    }
  }
