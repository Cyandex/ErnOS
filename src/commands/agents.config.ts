...
function resolveAgentName(cfg: ErnOSConfig, agentId: string) {
...
function resolveAgentModel(cfg: ErnOSConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ErnOSConfig): AgentSummary[] {
