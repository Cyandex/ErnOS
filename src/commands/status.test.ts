...
beforeAll(() => {
  envSnapshot = captureEnv(["ERNOS_PROFILE"]);
  process.env.ERNOS_PROFILE = "isolated";
});
...
vi.mock("../infra/ernos-root.js", () => ({
  resolveErnOSPackageRoot: vi.fn().mockResolvedValue("/tmp/ernos"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/ernos/pnpm-lock.yaml",
      markerPath: "/tmp/ernos/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.ernos.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.ernos.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("ErnOS status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("ernos status --all") ||
          l.includes("ernos --profile isolated status --all") ||
          l.includes("ernos status --all") ||
          l.includes("ernos --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ERNOS_GATEWAY_TOKEN = "abcd1234";
    try {
