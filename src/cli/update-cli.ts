...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update ErnOS and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["ernos update", "Update a source checkout (git)"],
        ["ernos update --channel beta", "Switch to beta channel (git + npm)"],
        ["ernos update --channel dev", "Switch to dev channel (git + npm)"],
        ["ernos update --tag beta", "One-off update to a dist-tag or version"],
        ["ernos update --dry-run", "Preview actions without changing anything"],
        ["ernos update --no-restart", "Update without restarting the service"],
        ["ernos update --json", "Output result as JSON"],
        ["ernos update --yes", "Non-interactive (accept downgrade prompts)"],
        ["ernos update wizard", "Interactive update wizard"],
        ["ernos --update", "Shorthand for ernos update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run ernos update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.ernos.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ernos update status", "Show channel + version status."],
          ["ernos update status --json", "JSON output."],
          ["ernos update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.ernos.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
