...
import type { ConfigFileSnapshot, ErnOSConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `ernos config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: ErnOSConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: ErnOSConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: { resolved: ErnOSConfig; config: ErnOSConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/ernos.json",
    exists: true,
...
function setSnapshot(resolved: ErnOSConfig, config: ErnOSConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: ErnOSConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
