...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "ernos-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "ernos-bundled-skills-test-"));
    process.env.ERNOS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
