    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ernos memory status", "Show index and provider status."],
          ["ernos memory index --force", "Force a full reindex."],
          ['ernos memory search --query "deployment notes"', "Search indexed memory entries."],
          ["ernos memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.ernos.ai/cli/memory")}\n`,
    );
