...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.ERNOS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.ERNOS_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<ErnOSConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live ErnOS docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  },
  {
    ...
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../ernos-cli.js");
      mod.registerErnOSCli(program);
    },
  },
  {
    name: "pairing",
  ...
  {
    name: "plugins",
    description: "Manage ErnOS plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
