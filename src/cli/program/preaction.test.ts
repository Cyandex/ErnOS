...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ERNOS_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ERNOS_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ERNOS_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("ernos");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ernos", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "ernos", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "ernos", "onboard"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "ernos", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "ernos", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "ernos", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ernos", "--version"],
    });
  ...
  it("hides banner when ERNOS_HIDE_BANNER is truthy", async () => {
    process.env.ERNOS_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ernos", "status"],
    });
