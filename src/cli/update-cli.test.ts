...
import type { ErnOSConfig, ConfigFileSnapshot } from "../config/types.ernos.js";
...
vi.mock("../infra/ernos-root.js", () => ({
  resolveErnOSPackageRoot: vi.fn(),
}));
...
const { resolveErnOSPackageRoot } = await import("../infra/ernos-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/ernos-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as ErnOSConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/ernos-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveErnOSPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("ernos-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveErnOSPackageRoot).mockClear();
    ...
    vi.mocked(resolveErnOSPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("ernos");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/ernos-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "ernos-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ErnOSConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveErnOSPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("ernos-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ERNOS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ERNOS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("ernos-update-wizard");
    await withEnvAsync({ ERNOS_GIT_DIR: tempDir }, async () => {
      setTty(true);
