...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("ernos node install"), formatCliCommand("ernos node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("ernos node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ERNOS_LOG_PREFIX: baseEnv.ERNOS_LOG_PREFIX ?? "node",
