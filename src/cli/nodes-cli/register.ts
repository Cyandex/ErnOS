    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ernos nodes status", "List known nodes with live status."],
          ["ernos nodes pairing pending", "Show pending node pairing requests."],
          ['ernos nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["ernos nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.ernos.ai/cli/nodes")}\n`,
    );
