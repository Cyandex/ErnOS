...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@ernos/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@ernos/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@ernos/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@ernos/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @ernos/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@ernos/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@ernos/plugin-alpha@1.2.3",
        installPath: "/tmp/ernos/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@ernos/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @ernos/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@ernos/plugin-alpha@latest",
      true,
      "/tmp/ernos/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@ernos/plugin-alpha@latest",
      installPath: "/tmp/ernos/extensions/alpha",
      version: "1.2.3",
