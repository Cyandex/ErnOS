...
import { resolveErnOSPackageRoot } from "../../infra/ernos-root.js";
...
const ERNOS_REPO_URL = "https://github.com/ernos/ernos.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "ernos";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("ernos@")) {
    return trimmed.slice("ernos@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.ERNOS_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ERNOS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ERNOS_GIT_DIR points at a non-git directory: ${params.dir}. Set ERNOS_GIT_DIR to an empty folder or an ernos checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ERNOS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "ernos.mjs");
