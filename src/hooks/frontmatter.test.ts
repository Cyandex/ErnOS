...
`;
    ...
    expect(parsed.ernos.emoji).toBe("💾");
    expect(parsed.ernos.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.ernos.emoji).toBe("📝");
    expect(parsed.ernos.events).toEqual(["command"]);
    expect(parsed.ernos.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.ernos.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"ernos": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"ernos": {"events": ["test"]}}');
  });
...
describe("resolveErnOSMetadata", () => {
  it("extracts ernos metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveErnOSMetadata(frontmatter);
  ...
  it("returns undefined when ernos key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        ernos: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ErnOS" },
            { id: "npm", kind: "npm", package: "@ernos/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@ernos/hook");
  });
...
`;
    ...
    const ernos = resolveErnOSMetadata(frontmatter);
    expect(ernos).toBeDefined();
    expect(ernos?.emoji).toBe("💾");
    expect(ernos?.events).toEqual(["command:new", "command:reset"]);
    expect(ernos?.requires?.config).toEqual(["workspace.dir"]);
    expect(ernos?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const ernos = resolveErnOSMetadata(frontmatter);
    expect(ernos?.emoji).toBe("disk");
    expect(ernos?.events).toEqual(["command:new"]);
  });
});
