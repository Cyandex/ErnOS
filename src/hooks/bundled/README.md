...
This directory contains hooks that ship with ErnOS. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.ernos/ernos.json`:
...
2. Restart gateway: `pkill -9 -f 'ernos.*gateway' && pnpm ernos gateway`
3. Enable the hook: `ernos hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.ernos.ai/automation/hooks
