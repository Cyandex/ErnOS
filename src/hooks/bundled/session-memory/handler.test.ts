...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => ErnOSConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("ernos-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): ErnOSConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
