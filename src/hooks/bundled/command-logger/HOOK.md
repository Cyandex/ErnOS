...
homepage: https://docs.ernos.ai/automation/hooks#command-logger
metadata:
  {
    "ernos":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ernos/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ernos/logs/commands.log ~/.ernos/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ernos`:
   ```
   /home/username/.ernos/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
