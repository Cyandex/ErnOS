...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ERNOS_BUNDLED_HOOKS_DIR"]);
    process.env.ERNOS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
