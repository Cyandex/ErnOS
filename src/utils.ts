...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".ernos");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ERNOS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ERNOS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ERNOS_STATE_DIR.
