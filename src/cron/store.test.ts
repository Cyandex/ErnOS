...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ERNOS_HOME for tilde expansion", () => {
    vi.stubEnv("ERNOS_HOME", "/srv/ernos-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/ernos-home", "cron", "jobs.json"));
  });
});
