...
describe("canvas host state dir defaults", () => {
  it("uses ERNOS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("ernos-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("ErnOS Canvas");
      } finally {
        await handler.close();
      }
    });
