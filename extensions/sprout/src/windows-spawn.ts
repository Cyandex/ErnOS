...
function resolveBinEntry(binField: string | Record<string, string> | undefined): string | null {
  ...
  const preferred = binField.sprout;
...
function resolveSproutScriptFromPackageJson(wrapperPath: string): string | null {
  const wrapperDir = path.dirname(wrapperPath);
  const packageDirs = [
    // Local install: <repo>/node_modules/.bin/sprout.cmd -> ../sprout
    path.resolve(wrapperDir, "..", "sprout"),
    // Global npm install: <npm-prefix>/sprout.cmd -> ./node_modules/sprout
    path.resolve(wrapperDir, "node_modules", "sprout"),
  ];
...
function resolveSproutScriptFromCmdShim(wrapperPath: string): string | null {
...
export function resolveWindowsSproutSpawn(
  execPath: string,
  argv: string[],
  ...
  const scriptPath =
    resolveSproutScriptFromCmdShim(resolvedExecPath) ??
    resolveSproutScriptFromPackageJson(resolvedExecPath);
  if (!scriptPath) {
    throw new Error(
      `${path.basename(resolvedExecPath)} wrapper resolved, but no Node entrypoint could be resolved without shell execution. Ensure Sprout is installed and runnable on PATH (prefer sprout.exe).`,
    );
  }
