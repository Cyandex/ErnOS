...
import type { ErnOSPluginApi, ErnOSPluginToolContext } from "../../../src/plugins/types.js";
import {
...
let createSproutTool: typeof import("./sprout-tool.js").createSproutTool;

function fakeApi(overrides: Partial<ErnOSPluginApi> = {}): ErnOSPluginApi {
  return {
    id: "sprout",
    name: "sprout",
    source: "test",
...
function fakeCtx(overrides: Partial<ErnOSPluginToolContext> = {}): ErnOSPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("sprout plugin tool", () => {
  let tempDir = "";
  const originalProcessState = snapshotPlatformPathEnv();

  beforeAll(async () => {
    ({ createSproutTool } = await import("./sprout-tool.js"));

    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-sprout-plugin-"));
  });
  ...
  it("runs sprout and returns parsed envelope in details", async () => {
    spawnState.queue.push({
      stdout: JSON.stringify({
        ok: true,
        status: "ok",
    ...
    const tool = createSproutTool(fakeApi());
  ...
  it("rejects invalid JSON from sprout", async () => {
  ...
  it("runs Windows cmd shims through Node without enabling shell", async () => {
    ...
    const shimScriptPath = path.join(tempDir, "shim-dist", "sprout-cli.cjs");
    const shimPath = path.join(tempDir, "shim-bin", "sprout.cmd");
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ErnOSPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
      return createSproutTool(api);
    };

    expect(factoryTool(fakeCtx({ sandboxed: true }))).toBeNull();
    expect(factoryTool(fakeCtx({ sandboxed: false }))?.name).toBe("sprout");
  });
});
