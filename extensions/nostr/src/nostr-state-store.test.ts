...
import type { PluginRuntime } from "ernos/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ERNOS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ernos-nostr-"));
  process.env.ERNOS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.ERNOS_STATE_DIR?.trim() || stateEnv.ERNOS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".ernos");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ERNOS_STATE_DIR = previous;
    }
