...
function resolveGatewayPort(cfg: ErnOSPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ERNOS_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.ERNOS_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: ErnOSPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: ErnOSPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the ErnOS iOS app:", "", "```", qrAscii, "```"].join("\n"),
                {
        ...
        return {
          text: [
            "Scan this QR code with the ErnOS iOS app:",
            "",
            "```",
            qrAscii,
            "```",
