...
import type { ErnOSPluginApi, ErnOSPluginService } from "ernos/plugin-sdk";
...
function normalizeDenyList(cfg: ErnOSPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ErnOSPluginApi["config"]): string[] {
...
export default function register(api: ErnOSPluginApi) {
  ...
  const timerService: ErnOSPluginService = {
