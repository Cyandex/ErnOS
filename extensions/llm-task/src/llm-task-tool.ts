...
import Ajv from "ajv";
import { resolvePreferredErnOSTmpDir } from "ernos/plugin-sdk";
// NOTE: This extension is intended to be bundled with ErnOS.
// When running from source (tests/dev), ErnOS internals live under src/.
...
export function createLlmTaskTool(api: ErnOSPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Sprout workflows via ernos.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(resolvePreferredErnOSTmpDir(), "ernos-llm-task-"));
