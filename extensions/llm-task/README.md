...
Designed to be called from workflow engines (for example, Sprout via
`ernos.invoke --each`) without adding new ErnOS code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Sprout) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on ErnOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ErnOS extension (like `sprout`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.ernos/extensions` as a standalone plugin directory.
