...
function resolveErnOSRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ERNOS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ERNOS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveErnOSRoot(), "dist", "extensionAPI.js");
