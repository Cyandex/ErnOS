...
metadata: { "ernos": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ErnOS `sessions_spawn`
- **File I/O** == ErnOS `read`/`write`
- **Remote fetch** == ErnOS `web_fetch` (or `exec` with curl when POST is required)
