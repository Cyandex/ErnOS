...
vi.mock("ernos/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("ernos/plugin-sdk")>("ernos/plugin-sdk");
  return {
    ...actual,
...
import type { ErnOSPluginServiceContext } from "ernos/plugin-sdk";
import { emitDiagnosticEvent } from "ernos/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/ernos-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): ErnOSPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("ernos.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("ernos.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("ernos.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("ernos.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("ernos.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("ernos.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("ernos.session.stuck")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("ernos.session.stuck_age_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("ernos.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("ernos.webhook.processed");
    expect(spanNames).toContain("ernos.message.processed");
    expect(spanNames).toContain("ernos.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["ernos.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("ernos.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "ernos.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["ernos.reason"]).toBe("string");
    expect(String(attrs?.["ernos.reason"])).not.toContain("ghp_abcdefghijklmnopqrstuvwxyz123456");
    await service.stop?.(ctx);
  });
});
