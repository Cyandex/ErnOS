...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["ernosPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["ernos", "pollId"]) ??
    readNestedString(value, ["ernos", "poll", "id"]) ??
    readNestedString(value, ["data", "ernosPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "ernos", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "ernos poll vote",
          displayText: "Vote recorded",
          value: { ernosPollId: pollId, pollId },
        },
      },
    ],
  };
