import type { AnyAgentTool, ErnOSPluginApi } from "ernos/plugin-sdk";
...
export function createToolFactoryHarness(cfg: ErnOSPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<ErnOSPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
