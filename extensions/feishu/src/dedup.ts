...
import { createDedupeCache, createPersistentDedupe } from "ernos/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.ERNOS_STATE_DIR?.trim() || env.ERNOS_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["ernos-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".ernos");
}
