 ...
 */
function setTwitchAccount(cfg: ErnOSConfig, account: Partial<TwitchAccountConfig>): ErnOSConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ERNOS_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: ErnOSConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var ERNOS_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ERNOS_TWITCH_ACCESS_TOKEN?.trim();
