# Google Gemini CLI Auth (ErnOS plugin)
...
```bash
ernos plugins enable google-gemini-cli-auth
```
...
```bash
ernos models auth login --provider google-gemini-cli --set-default
```
...
- `ERNOS_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `ERNOS_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
