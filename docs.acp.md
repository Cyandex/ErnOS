...
This document describes how the ErnOS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`ernos acp` exposes an ACP agent over stdio and forwards prompts to a running
ErnOS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `ernos acp` and speaks ACP messages over stdio.
...
`ernos acp` resolves the Gateway URL and auth from CLI flags or config:
