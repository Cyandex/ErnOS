...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/ernos/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/ernos");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/ernos/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ERNOS_CONTROL_UI_BASE_PATH__ = "/ernos";
    const app = mountApp("/ernos/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/ernos");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/ernos/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("ernos.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
