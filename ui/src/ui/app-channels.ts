...
export async function handleWhatsAppStart(host: ErnOSApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ErnOSApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ErnOSApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ErnOSApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ErnOSApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ErnOSApp): string {
...
function resolveGatewayHttpAuthHeader(host: ErnOSApp): string | null {
...
function buildGatewayHttpHeaders(host: ErnOSApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ErnOSApp) {
...
export function handleNostrProfileToggleAdvanced(host: ErnOSApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ErnOSApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ErnOSApp) {
  const state = host.nostrProfileFormState;
