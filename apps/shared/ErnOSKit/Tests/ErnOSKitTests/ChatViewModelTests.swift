...
private final class TestChatTransport: @unchecked Sendable, ErnOSChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ErnOSChatHistoryPayload]
    private let sessionsResponses: [ErnOSChatSessionsListResponse]

    private let stream: AsyncStream<ErnOSChatTransportEvent>
    private let continuation: AsyncStream<ErnOSChatTransportEvent>.Continuation

    init(
        historyResponses: [ErnOSChatHistoryPayload],
        sessionsResponses: [ErnOSChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ErnOSChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ErnOSChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ErnOSChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ErnOSChatAttachmentPayload]) async throws -> ErnOSChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ErnOSChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ErnOSChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ErnOSChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ErnOSChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ErnOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ErnOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ErnOSChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ErnOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ErnOSChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ErnOSChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ErnOSChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
