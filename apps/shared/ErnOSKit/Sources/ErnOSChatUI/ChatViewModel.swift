...
private let chatUILogger = Logger(subsystem: "ai.ernos", category: "ErnOSChatUI")

@MainActor
@Observable
public final class ErnOSChatViewModel {
    public private(set) var messages: [ErnOSChatMessage] = []
    ...
    public var attachments: [ErnOSPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ErnOSChatPendingToolCall] = []
    public private(set) var sessions: [ErnOSChatSessionEntry] = []
    private let transport: any ErnOSChatTransport
    ...
    private var pendingToolCallsById: [String: ErnOSChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ErnOSChatTransport) {
    ...
    public var sessionChoices: [ErnOSChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [ErnOSChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ErnOSPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ErnOSChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ErnOSChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ErnOSChatMessage) -> ErnOSChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ErnOSChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ErnOSChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: ErnOSChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ErnOSChatMessage],
        incoming: [ErnOSChatMessage]) -> [ErnOSChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ErnOSChatMessage]) -> [ErnOSChatMessage] {
        var result: [ErnOSChatMessage] = []
    ...
    private static func dedupeKey(for message: ErnOSChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ErnOSChatMessageContent] = [
            ErnOSChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ErnOSChatAttachmentPayload in
            ErnOSChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ErnOSChatSessionEntry {
        ErnOSChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ErnOSChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ErnOSChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ErnOSAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ErnOSChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ErnOSPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
