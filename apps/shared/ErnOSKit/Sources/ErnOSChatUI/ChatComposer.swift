...
@MainActor
struct ErnOSChatComposer: View {
    @Bindable var viewModel: ErnOSChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ErnOSChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ErnOSChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ErnOSPendingAttachment.id)
                { (att: ErnOSPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ErnOSPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ErnOSChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ErnOSChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ErnOSChatTheme.subtleCard)
        .clipShape(Capsule())
    }
