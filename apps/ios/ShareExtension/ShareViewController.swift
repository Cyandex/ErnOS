...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.ernos.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to ErnOS", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to ErnOS gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "ErnOSShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "ErnOS is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("ernos-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
