...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ERNOS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ErnOSTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.ernos", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ErnOSTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ErnOSTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ErnOSTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ErnOSTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ErnOSTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ErnOSTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ErnOSTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ErnOSTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ErnOSTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: ErnOSAgentEventPayload.self
            ) else {
                continue
            }
