...
struct IOSGatewayChatTransport: ErnOSChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.ernos", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> ErnOSChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ErnOSChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ErnOSChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ErnOSChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ErnOSChatAttachmentPayload]) async throws -> ErnOSChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ErnOSChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ErnOSChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ErnOSGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ErnOSGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ErnOSChatEventPayload.self)
                        {
