...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ErnOSCapability.canvas.rawValue, ErnOSCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ErnOSCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ErnOSCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ErnOSLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ErnOSCapability.location.rawValue) }

        caps.append(ErnOSCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(ErnOSCapability.watch.rawValue)
        }
        caps.append(ErnOSCapability.photos.rawValue)
        caps.append(ErnOSCapability.contacts.rawValue)
        caps.append(ErnOSCapability.calendar.rawValue)
        caps.append(ErnOSCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(ErnOSCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ErnOSScreenCommand.record.rawValue,
            ErnOSSystemCommand.notify.rawValue,
            ErnOSChatCommand.push.rawValue,
        ...
        if caps.contains(ErnOSCapability.camera.rawValue) {
            commands.append(ErnOSCameraCommand.list.rawValue)
            commands.append(ErnOSCameraCommand.snap.rawValue)
            commands.append(ErnOSCameraCommand.clip.rawValue)
        }
        if caps.contains(ErnOSCapability.location.rawValue) {
            commands.append(ErnOSLocationCommand.get.rawValue)
        }
        if caps.contains(ErnOSCapability.device.rawValue) {
            commands.append(ErnOSDeviceCommand.status.rawValue)
            commands.append(ErnOSDeviceCommand.info.rawValue)
        }
        if caps.contains(ErnOSCapability.watch.rawValue) {
            commands.append(ErnOSWatchCommand.status.rawValue)
            commands.append(ErnOSWatchCommand.notify.rawValue)
        }
        if caps.contains(ErnOSCapability.photos.rawValue) {
            commands.append(ErnOSPhotosCommand.latest.rawValue)
        }
        if caps.contains(ErnOSCapability.contacts.rawValue) {
            commands.append(ErnOSContactsCommand.search.rawValue)
            commands.append(ErnOSContactsCommand.add.rawValue)
        }
        if caps.contains(ErnOSCapability.calendar.rawValue) {
            commands.append(ErnOSCalendarCommand.events.rawValue)
            commands.append(ErnOSCalendarCommand.add.rawValue)
        }
        if caps.contains(ErnOSCapability.reminders.rawValue) {
            commands.append(ErnOSRemindersCommand.list.rawValue)
            commands.append(ErnOSRemindersCommand.add.rawValue)
        }
        if caps.contains(ErnOSCapability.motion.rawValue) {
            commands.append(ErnOSMotionCommand.activity.rawValue)
            commands.append(ErnOSMotionCommand.pedometer.rawValue)
        }
