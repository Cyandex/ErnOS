...
final class RemindersService: RemindersServicing {
    func list(params: ErnOSRemindersListParams) async throws -> ErnOSRemindersListPayload {
        ...
        let payload: [ErnOSReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return ErnOSRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ErnOSRemindersAddParams) async throws -> ErnOSRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ErnOSReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ErnOSRemindersAddPayload(reminder: payload)
    }
