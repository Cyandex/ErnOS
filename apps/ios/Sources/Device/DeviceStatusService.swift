...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ErnOSDeviceStatusPayload {
        ...
        return ErnOSDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> ErnOSBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ErnOSBatteryState = switch device.batteryState {
        ...
        return ErnOSBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ErnOSThermalStatusPayload {
        let state: ErnOSThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ErnOSThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ErnOSStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ErnOSStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
