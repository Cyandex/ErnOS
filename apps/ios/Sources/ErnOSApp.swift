...
@MainActor
final class ErnOSAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.ernos.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.ernos.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.ernos.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "ernos.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "ernos.watch.promptId"
    static let sessionKeyKey = "ernos.watch.sessionKey"
    static let actionPrimaryIDKey = "ernos.watch.action.primary.id"
    static let actionPrimaryLabelKey = "ernos.watch.action.primary.label"
    static let actionSecondaryIDKey = "ernos.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "ernos.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "ernos.watch.action.primary"
    static let actionSecondaryIdentifier = "ernos.watch.action.secondary"
    static let actionIdentifierPrefix = "ernos.watch.action."
    static let actionIDKeyPrefix = "ernos.watch.action.id."
    static let actionLabelKeyPrefix = "ernos.watch.action.label."
    static let categoryPrefix = "ernos.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ErnOSWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ErnOSWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ErnOSWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "ErnOS" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ErnOSWatchAction]) -> [UNNotificationAction] {
...
struct ErnOSApp: App {
    ...
    @UIApplicationDelegateAdaptor(ErnOSAppDelegate.self) private var appDelegate
...
extension ErnOSApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("ErnOS: installing uncaught exception handler")
