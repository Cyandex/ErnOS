...
final class MotionService: MotionServicing {
    func activities(params: ErnOSMotionActivityParams) async throws -> ErnOSMotionActivityPayload {
        ...
        let mapped: [ErnOSMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ErnOSMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ErnOSPedometerParams) async throws -> ErnOSPedometerPayload {
        ...
        let payload: ErnOSPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ErnOSPedometerPayload(
