...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ERNOS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/ernos")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("ernos-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("ernos-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/ernos/ (rolling or stdout), or nil if none exist.
