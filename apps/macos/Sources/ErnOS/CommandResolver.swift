...
enum CommandResolver {
    private static let projectRootDefaultsKey = "ernos.gatewayProjectRootPath"
    private static let helperName = "ernos"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let ernosEntry = root.appendingPathComponent("ernos.mjs").path
        if FileManager().isReadableFile(atPath: ernosEntry) { return ernosEntry }
        let binEntry = root.appendingPathComponent("bin/ernos.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/ernos")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let ernosPaths = self.ernosManagedPaths(home: home)
        if !ernosPaths.isEmpty {
            extras.insert(contentsOf: ernosPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + ernosPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func ernosManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".ernos"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func ernosExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectErnOSExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("ernos.mjs").path,
            root.appendingPathComponent("bin/ernos.js").path,
        ]
    ...
    static func hasAnyErnOSInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.ernosExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let ernosPath = self.projectErnOSExecutable(projectRoot: root) {
            return [ernosPath, subcommand] + extraArgs
        }
        if let ernosPath = self.ernosExecutable(searchPaths: searchPaths) {
            return [ernosPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "ernos", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            ernos entrypoint missing (looked for dist/index.js or ernos.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real ernos CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/ernos"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/ernos.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/ernos.mjs"
            node "$PRJ/ernos.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/ernos.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/ernos.js"
            node "$PRJ/bin/ernos.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent ernos"
          pnpm --silent ernos \(quotedArgs);
        else
          echo "ernos CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ErnOSConfigFile.loadDict()
