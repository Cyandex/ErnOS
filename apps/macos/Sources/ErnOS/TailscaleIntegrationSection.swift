...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.ernos/ernos.json. Prefer ERNOS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.ernos/ernos.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.ernos/ernos.json. Restart the gateway to apply."
        }
