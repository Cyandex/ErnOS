...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ErnOSProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.ernos", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ErnOSKit.AnyCodable]()) {
                $0[$1.key] = ErnOSKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ErnOSProtocol.AnyCodable?) -> [String: ErnOSProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ErnOSProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ErnOSKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ErnOSProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("ernos.control.heartbeat")
    static let controlAgentEvent = Notification.Name("ernos.control.agent")
}
