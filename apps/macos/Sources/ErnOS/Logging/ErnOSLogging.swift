...
enum ErnOSLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ErnOSOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ErnOSFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ErnOSLogging.bootstrapIfNeeded()
        let label = ErnOSLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ErnOSOSLogHandler: LogHandler {
...
struct ErnOSFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ErnOSLogging.parseLabel(self.label)
