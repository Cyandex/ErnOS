...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.ernos.mac`) even if the shipped
    ...
    {
        if environment["ERNOS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "ernos.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "ernos.nixMode") { return true }
