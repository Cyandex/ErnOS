...
enum DebugActions {
    private static let verboseDefaultsKey = "ernos.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "ernos.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ErnOSPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ErnOS", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ErnOSPaths.configURL
        guard
