...
private let webChatSwiftLogger = Logger(subsystem: "ai.ernos", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ErnOSChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ErnOSChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ErnOSChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ErnOSChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ErnOSGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ErnOSChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ErnOSChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ErnOSChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ErnOSChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ErnOSChatView>) -> NSViewController
    {
