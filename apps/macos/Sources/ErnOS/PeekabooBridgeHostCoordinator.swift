...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.ernos", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ErnOSPeekabooBridgeServices?
    private static var ernosSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ErnOS", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ErnOSPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.ernosSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.ernosSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ErnOSPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.ernos.peekaboo")
