...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.ernos", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ErnOSSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ErnOSSessionPreviewEntry,
    ...
    private static func cache(payload: ErnOSSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ErnOSChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ErnOSKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ErnOSKit.AnyCodable]) -> [ErnOSChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ErnOSChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ErnOSChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ErnOSChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ErnOSChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ErnOSChatMessage) -> String? {
