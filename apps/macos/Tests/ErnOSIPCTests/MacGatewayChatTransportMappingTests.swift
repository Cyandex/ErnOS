...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ErnOSProtocol.AnyCodable(["ok": ErnOSProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ErnOSProtocol.AnyCodable(["ok": ErnOSProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ErnOSProtocol.AnyCodable([
            "runId": ErnOSProtocol.AnyCodable("run-1"),
            "sessionKey": ErnOSProtocol.AnyCodable("main"),
            "state": ErnOSProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ErnOSProtocol.AnyCodable(["a": ErnOSProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
