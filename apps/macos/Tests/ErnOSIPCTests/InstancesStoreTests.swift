...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ErnOSProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ErnOSProtocol.AnyCodable] = [
            "presence": .init([ErnOSProtocol.AnyCodable(entry)]),
        ]
        let payload = ErnOSProtocol.AnyCodable(payloadMap)
