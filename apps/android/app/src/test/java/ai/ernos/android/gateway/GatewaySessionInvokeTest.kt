...
class GatewaySessionInvokeTest {
  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
                ...
                object : WebSocketListener() {
                  ...
                  override fun onMessage(webSocket: WebSocket, text: String) {
                    ...
                    when (method) {
                      "connect" -> {
                        webSocket.send(
                          """{"type":"res","id":"$id","ok":true,"payload":{"canvasHostUrl":"http://127.0.0.1/__ernos__/cap/old-cap","snapshot":{"sessionDefaults":{"mainSessionKey":"main"}}}}""",
                        )
                      }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__ernos__/cap/new-cap",
