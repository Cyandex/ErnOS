#!/usr/bin/env bash
# ClawDock - Docker helpers for ErnOS
# Inspired by Simon Willison's "Running ErnOS in Docker"
# https://til.simonwillison.net/llms/ernos-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/ernos/ernos/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for ErnOS
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/ernos"
  "${HOME}/workspace/ernos"
  "${HOME}/projects/ernos"
  "${HOME}/dev/ernos"
  "${HOME}/code/ernos"
  "${HOME}/src/ernos"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🌱 Found ErnOS at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/ernos"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ErnOS not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/ernos/ernos.git ~/ernos"
    echo "  cd ~/ernos && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ERNOS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d ernos-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart ernos-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f ernos-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec ernos-gateway \
    bash -c 'echo "alias ernos=\"./ernos.mjs\"" > /tmp/.bashrc_ernos && bash --rcfile /tmp/.bashrc_ernos'
}
...
clawdock-cli() {
  _clawdock_compose run --rm ernos-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build ernos-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ERNOS_GATEWAY_TOKEN=$token" ernos-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" ernos-gateway \
    bash -c './ernos.mjs config set gateway.remote.token "$TOKEN" && ./ernos.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec ernos-gateway \
    bash -c "./ernos.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart ernos-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm ernos-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec ernos-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'ernos config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🌱 ClawDock - Docker Helpers for ErnOS${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (ernos alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to ernos project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.ernos)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ernos channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ernos status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.ernos.ai${_CLR_RESET}"
  echo ""
}
