...
# Checks both Claude Code and ErnOS auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ERNOS_AUTH="$HOME/.ernos/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    ernos models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    ernos_status=$(check_ernos_auth 2>/dev/null || true)

    claude_expires=0
    ernos_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        ernos_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        ernos_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ERNOS_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$ernos_status" \
        --arg be "$ernos_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            ernos: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$ernos_status" == EXPIRED* ]] || [[ "$ernos_status" == MISSING* ]]; then
        echo "ERNOS_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$ernos_status" == EXPIRING* ]]; then
        echo "ERNOS_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "ErnOS Auth (~/.ernos/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ERNOS_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ERNOS_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'ernos doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active ernos >/dev/null 2>&1; then
    echo -e "ErnOS service: ${GREEN}running${NC}"
else
    echo -e "ErnOS service: ${RED}NOT running${NC}"
fi
