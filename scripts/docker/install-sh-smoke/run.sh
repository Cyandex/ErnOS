...
INSTALL_URL="${ERNOS_INSTALL_URL:-https://ernos.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ERNOS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ERNOS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="ernos"
PACKAGE_NAME="${ERNOS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ERNOS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ERNOS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ERNOS_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
