...
SERVER="${ERNOS_SERVER:-${ERNOS_SERVER:-l36}}"

# Check auth status
termux-toast "Checking ErnOS auth..."

STATUS=$(ssh "$SERVER" '$HOME/ernos/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/ernos/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ERNOS_EXPIRING|ERNOS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/ernos/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ERNOS_EXPIRED|ERNOS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ErnOS won't work until re-auth"
                ;;
        esac
        ;;
