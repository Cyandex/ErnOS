...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/ernos/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ErnOS Re-Auth" -c "After getting key, run: ssh $SERVER '~/ernos/scripts/mobile-reauth.sh'" --id ernos-auth
        ;;
    *)
