...
BASE_IMAGE="${BASE_IMAGE:-ernos-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-ernos-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=ernos.sandbox=1)
NOTE
