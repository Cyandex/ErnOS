 ...
 * `ernos update`. Use it to verify the completion installation flow
...
\`ernos update\`, \`ernos doctor\`, and \`ernos onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# ErnOS Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`ernos completion --install\` later to enable.`));
    return;
  }
