...
IMAGE_NAME="ernos-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="ernos-net-e2e-$$"
GW_NAME="ernos-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ERNOS_GATEWAY_TOKEN=$TOKEN" \
  -e "ERNOS_SKIP_CHANNELS=1" \
  -e "ERNOS_SKIP_GMAIL_WATCHER=1" \
  -e "ERNOS_SKIP_CRON=1" \
  -e "ERNOS_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
