#!/bin/bash
# Setup ErnOS Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'ernos-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ErnOS message: Send warning via ErnOS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/ernos-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/ernos-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/ernos-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now ernos-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status ernos-auth-monitor.timer"
echo ""
echo "Quick commands:"
