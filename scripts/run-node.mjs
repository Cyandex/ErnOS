...
const shouldBuild = (deps) => {
  if (deps.env.ERNOS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ERNOS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[ernos] ${message}\n`);
};

const runErnOS = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["ernos.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
