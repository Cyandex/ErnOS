...
# Build and bundle ErnOS into a minimal .app we can open.
# Outputs to dist/ErnOS.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ErnOS.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ErnOS"
BUNDLE_ID="${BUNDLE_ID:-ai.ernos.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/ernos/ernos/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ErnOS/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ErnOSBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ErnOSGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ErnOS"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ErnOS"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ErnOS"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ErnOS" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ErnOS/Resources/ErnOS.icns" "$APP_ROOT/Contents/Resources/ErnOS.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ErnOS/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ErnOSKit resources"
ERNOSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ErnOSKit_ErnOSKit.bundle"
if [ -d "$ERNOSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ErnOSKit_ErnOSKit.bundle"
  cp -R "$ERNOSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ErnOSKit_ErnOSKit.bundle"
else
  echo "WARN: ErnOSKit resource bundle not found at $ERNOSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ErnOS"
killall -q ErnOS 2>/dev/null || true
