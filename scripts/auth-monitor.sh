...
#
# Suggested cron: */30 * * * * /home/admin/ernos/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ErnOS notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., ernos-alerts)
...
STATE_FILE="$HOME/.ernos/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ErnOS if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use ernos
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ErnOS to $NOTIFY_PHONE..."
            ernos send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ErnOS is down. Run: ssh l36 '~/ernos/scripts/mobile-reauth.sh'" "urgent"
    exit 1
