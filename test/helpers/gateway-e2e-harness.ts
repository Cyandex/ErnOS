...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ernos-e2e-${name}-`));
  const configDir = path.join(homeDir, ".ernos");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ERNOS_SKIP_PROVIDERS: "1",
          ERNOS_SKIP_GMAIL_WATCHER: "1",
          ERNOS_SKIP_CRON: "1",
          ERNOS_SKIP_BROWSER_CONTROL_SERVER: "1",
          ERNOS_SKIP_CANVAS_HOST: "1",
          ERNOS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
