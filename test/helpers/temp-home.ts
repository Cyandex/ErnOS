...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ERNOS_HOME.
  delete process.env.ERNOS_HOME;
  process.env.ERNOS_STATE_DIR = path.join(base, ".ernos");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "ernos-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ernos", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
