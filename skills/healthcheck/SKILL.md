---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ErnOS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ErnOS cron scheduling for periodic checks, or version status checks on a machine running ErnOS (laptop, workstation, Pi, VPS).
---

# ErnOS Host Hardening

## Overview

Assess and harden the host running ErnOS, then align it to a user-defined risk tolerance without breaking access. Use ErnOS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ErnOS changes the host firewall, SSH, or OS updates; it does not.
...
5. ErnOS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ErnOS can access sensitive data.
...
### 2) Run ErnOS security audits (read-only)

As part of the default read-only checks, run `ernos security audit --deep`. Only offer alternatives if the user requests them:

1. `ernos security audit` (faster, non-probing)
2. `ernos security audit --json` (structured output)

Offer to apply ErnOS safe defaults (numbered):

1. `ernos security audit --fix`

Be explicit that `--fix` only tightens ErnOS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ErnOS version/update status (read-only)

As part of the default read-only checks, run `ernos update status`.
...
- Credential hygiene notes (location of ErnOS creds, prefer disk encryption)
...
After ErnOS install or first hardening pass, run at least one baseline audit and version check:

- `ernos security audit`
- `ernos security audit --deep`
- `ernos update status`

Ongoing monitoring is recommended. Use the ErnOS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `ernos cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `ernos update status`
...
Before creating, `ernos cron list` and match on exact `name`. If found, `ernos cron edit <id> ...`.
If not found, `ernos cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `ernos update status` (preferred for source checkouts and channels)
2. `npm view ernos version` (published npm version)

## ErnOS command accuracy

Use only supported commands and flags:

- `ernos security audit [--deep] [--fix] [--json]`
- `ernos status` / `ernos status --deep`
- `ernos health --json`
- `ernos update status`
- `ernos cron add|list|runs|run`

Do not invent CLI flags or imply ErnOS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ErnOS compaction:
