---
summary: "Mattermost bot setup and ErnOS config"
read_when:
...
```bash
ernos plugins install @ernos/mattermost
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/mattermost
```
...
4. Configure ErnOS and start the gateway.
...
- Approve via:
  - `ernos pairing list mattermost`
  - `ernos pairing approve mattermost <CODE>`
...
Use these target formats with `ernos message send` or cron/webhooks:
