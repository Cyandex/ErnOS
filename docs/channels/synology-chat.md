---
summary: "Synology Chat webhook setup and ErnOS config"
read_when:
  - Setting up Synology Chat with ErnOS
...
```bash
ernos plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your ErnOS gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in ErnOS.
...
- Pairing approvals work with:
  - `ernos pairing list synology-chat`
  - `ernos pairing approve synology-chat <CODE>`
...
```bash
ernos message send --channel synology-chat --target 123456 --text "Hello from ErnOS"
ernos message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
