...
read_when:
  - You want ErnOS to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ErnOS to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`ernos onboard`) and `ernos channels add` list optional channel plugins.
...
```bash
ernos plugins install @ernos/nostr
```

Use a local checkout (dev workflows):

```bash
ernos plugins install --link <path-to-ernos>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "ernos@example.com",
        "lud16": "ernos@example.com"
      }
    }
  }
}
