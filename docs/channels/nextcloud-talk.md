...
```bash
ernos plugins install @ernos/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ErnOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `ernos pairing list nextcloud-talk`
  - `ernos pairing approve nextcloud-talk <CODE>`
