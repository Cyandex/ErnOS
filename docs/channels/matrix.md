...
Matrix is an open, decentralized messaging protocol. ErnOS connects as a Matrix **user**
...
```bash
ernos plugins install @ernos/matrix
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `ernos plugins install @ernos/matrix`
   - From a local checkout: `ernos plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ErnOS calls the same
     login endpoint, stores the access token in `~/.ernos/credentials/matrix/credentials.json`,
...
- On first connection, ErnOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ErnOS logs a warning.
...
`~/.ernos/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `ernos pairing list matrix`
  - `ernos pairing approve matrix <CODE>`
...
- On startup, ErnOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
