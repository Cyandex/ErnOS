...
Tlon is a decentralized messenger built on Urbit. ErnOS connects to your Urbit ship and can
...
```bash
ernos plugins install @ernos/tlon
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/tlon
```
...
By default, ErnOS blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `ernos message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ErnOS replies in-thread.
