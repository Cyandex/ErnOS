...
read_when:
  - Setting up Twitch chat integration for ErnOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ErnOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ernos plugins install @ernos/twitch
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ERNOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ernos", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ERNOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
ERNOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
