...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ErnOS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
ernos plugins install @ernos/feishu
```

Local checkout (when running from a git repo):

```bash
ernos plugins install ./extensions/feishu
```
...
If you just installed ErnOS, run the wizard:
...
2. Configuring app credentials in ErnOS
...
1. You already ran `ernos channels add` for Feishu
2. The gateway is running (`ernos gateway status`)
...
```bash
ernos pairing approve feishu <CODE>
```
...
2. Run `ernos logs --follow` and look for `chat_id`
...
2. Run `ernos logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `ernos gateway status`  | Show gateway status           |
| `ernos gateway install` | Install/start gateway service |
| `ernos gateway stop`    | Stop gateway service          |
| `ernos gateway restart` | Restart gateway service       |
| `ernos logs --follow`   | Tail gateway logs             |
...
4. Check logs: `ernos logs --follow`
...
5. Ensure the gateway is running: `ernos gateway status`
6. Check logs: `ernos logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.ernos/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.ernos/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
