...
- Publish as `ernos/plugin-sdk` (or export from core under `ernos/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ErnOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ErnOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ErnOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ErnOSConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ErnOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ErnOSConfig): string;
  };
};
```
...
- Introduce `ernos/plugin-sdk`.
- Add `api.runtime` to `ErnOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `ernosRuntime: ">=2026.2.0"`).
