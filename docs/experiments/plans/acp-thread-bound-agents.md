...
This plan defines how ErnOS should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- ErnOS core owns ACP control plane concerns
...
ErnOS should not reimplement ACP transport internals in core.
ErnOS should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in ErnOS, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under ErnOS state dir
...
- ACP event stream is normalized to ErnOS reply chunks
...
For the first production backend (`extensions/acpx`), ErnOS and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses ErnOS `sessionKey`)
...
- ErnOS consumes ndjson events from `acpx --format json --json-strict`
