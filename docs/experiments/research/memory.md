...
read_when:
  - Designing workspace memory (~/.ernos/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ErnOS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.ernos/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.ernos/workspace` as canonical human-readable memory.
...
```
~/.ernos/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ErnOS**, but keep a separable core library.

### Why integrate into ErnOS?
...
- You want the agent itself to call the tools:
  - `ernos memory recall "…" --k 25 --since 30d`
  - `ernos memory reflect --since 7d`
...
Pragmatic take for `~/.ernos/workspace`:
