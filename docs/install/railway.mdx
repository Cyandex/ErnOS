...
Deploy ErnOS on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/ernos-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/ernos` — Control UI

## What you get

- Hosted ErnOS Gateway + Control UI
...
- `ERNOS_STATE_DIR=/data/.ernos` (recommended)
- `ERNOS_WORKSPACE_DIR=/data/workspace` (recommended)
- `ERNOS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ErnOS state + workspace so you can migrate to another host without losing config or memory.
