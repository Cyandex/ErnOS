...
Deploy ErnOS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/ernos/ernos)
...
repository configures everything needed to run ErnOS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ERNOS_STATE_DIR
        value: /data/.ernos
      - key: ERNOS_WORKSPACE_DIR
        value: /data/workspace
      - key: ERNOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/ernos`.
...
If you use the original ErnOS repository, Render will not auto-deploy your ErnOS. To update it, run a manual Blueprint sync from the dashboard.
...
For ErnOS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ErnOS host.
