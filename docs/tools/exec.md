...
- `gateway`/`node` approvals are controlled by `~/.ernos/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  ErnOS prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`ernos security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `ernos doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
