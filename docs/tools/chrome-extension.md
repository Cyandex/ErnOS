---
summary: "Chrome extension: let ErnOS drive your existing Chrome tab"
read_when:
...
The ErnOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate ernos-managed Chrome profile.
...
ErnOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ErnOS release (npm package) as static files. There is no separate “build” step.

After upgrading ErnOS:

- Re-run `ernos browser extension install` to refresh the installed files under your ErnOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

ErnOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ERNOS_GATEWAY_TOKEN`)
...
- Open the tab you want ErnOS to control.
...
- `ON`: attached; ErnOS can drive that tab.
...
Debugging: `ernos sandbox explain`
...
`ernos browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `ernos browser extension install` first to copy the extension to a stable location under your ErnOS state directory.
...
- **This is not isolated** like the dedicated ernos-managed profile.
