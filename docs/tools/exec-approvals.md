...
- **gateway host** → `ernos` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ernos/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.ernos/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `ernos approvals allowlist ...`).
- `ernos security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `ernos doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.ernos/exec-approvals.json` directly.

CLI: `ernos approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
