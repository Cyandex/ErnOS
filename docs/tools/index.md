---
summary: "Agent tool surface for ErnOS (browser, canvas, nodes, message, cron) replacing legacy `ernos-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `ernos-*` skills
title: "Tools"
---

# Tools (ErnOS)

ErnOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `ernos-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `ernos.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ErnOS logs a warning and ignores the allowlist so core tools stay available.
...
- [Sprout](/tools/sprout): typed workflow runtime with resumable approvals (requires the Sprout CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`ernos node run`).
...
ErnOS tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `ernos configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ErnOS-managed browser.
...
- Quick smoke: `ernos nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `ernos gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, ErnOS uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, ErnOS clamps `tools.sessions.visibility` to `tree`.
