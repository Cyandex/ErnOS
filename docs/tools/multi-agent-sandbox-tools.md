...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `ernos sandbox explain`.
...
Legacy `agent.*` configs are migrated by `ernos doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=ernos-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ERNOS_STATE_DIR:-$HOME/.ernos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
