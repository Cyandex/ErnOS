...
ErnOS supports **camera capture** for agent workflows:
...
```bash
ernos nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
ernos nodes camera snap --node <id> --facing front
ernos nodes camera clip --node <id> --duration 3000
ernos nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`ernos.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `ernos` CLI to invoke camera commands on the macOS node.

Examples:

```bash
ernos nodes camera list --node <id>            # list camera ids
ernos nodes camera snap --node <id>            # prints MEDIA:<path>
ernos nodes camera snap --node <id> --max-width 1280
ernos nodes camera snap --node <id> --delay-ms 2000
ernos nodes camera snap --node <id> --device-id <id>
ernos nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
ernos nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
ernos nodes camera clip --node <id> --device-id <id>
ernos nodes camera clip --node <id> --no-audio
```

Notes:

- `ernos nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
ernos nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
