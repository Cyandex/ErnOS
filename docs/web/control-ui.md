...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/ernos`)
...
If the page fails to load, start the Gateway first: `ernos gateway`.
...
you revoke it with `ernos devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.ernos/ernos.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop ernos`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ErnOS
...
ErnOS **blocks** Control UI connections without device identity.
