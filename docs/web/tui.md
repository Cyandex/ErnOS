...
```bash
ernos tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `ernos tui --deliver`
...
- Check the Gateway logs: `ernos logs --follow`.
- Confirm the agent can run: `ernos status` and `ernos models status`.
...
- No agents in picker: check `ernos agents list` and your routing config.
