...
read_when:
  ...
  - Wiring external systems into ErnOS
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your ErnOS config directory (typically `~/.ernos/hooks/transforms`).
...
- `ernos webhooks gmail setup` writes `hooks.gmail` config for `ernos webhooks gmail run`.
