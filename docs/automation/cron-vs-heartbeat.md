...
ernos cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
ernos cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
ernos cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Sprout: Deterministic workflows with approvals

Sprout is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
...
- **Sprout** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Sprout.
For ad-hoc workflows, call Sprout directly.

### Operational notes (from the code)

- Sprout runs as a **local subprocess** (`sprout` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["sprout"]` (recommended).
- Sprout expects the `sprout` CLI to be available on `PATH`.

See [Sprout](/tools/sprout) for full usage and examples.
