...
- Jobs persist under `~/.ernos/cron/` so restarts don’t lose schedules.
...
ernos cron list
ernos cron run <job-id>
ernos cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.ernos/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `ernos cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, ErnOS applies a
...
If `delivery` is omitted for isolated jobs, ErnOS defaults to `announce`.
...
- Job store: `~/.ernos/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.ernos/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, ErnOS archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `ernos cron runs` and adjust retention before logs become large
...
ernos cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
ernos cron edit <jobId> --agent ops
ernos cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
ernos cron run <jobId>
ernos cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
ernos cron edit <jobId> \
...
```bash
ernos system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`ernos system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ERNOS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ErnOS applies exponential retry backoff for recurring jobs after consecutive errors:
