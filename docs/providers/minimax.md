---
summary: "Use MiniMax M2.1 in ErnOS"
read_when:
  - You want MiniMax models in ErnOS
...
```bash
ernos plugins enable minimax-portal-auth  # skip if already loaded.
ernos gateway restart  # restart if gateway is already running
ernos onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ernos/ernos/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `ernos.json`:
...
- Use `ernos models list` and `ernos models set minimax/MiniMax-M2.1` to switch.
...
- Running `ernos configure` and selecting **MiniMax M2.1**, or
