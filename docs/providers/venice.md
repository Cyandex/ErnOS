---
summary: "Use Venice AI privacy-focused models in ErnOS"
read_when:
  - You want privacy-focused inference in ErnOS
...
```bash
ernos onboard --auth-choice venice-api-key
```
...
```bash
ernos agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, ErnOS shows all available Venice models. Pick based on your needs:
...
```bash
ernos models set venice/claude-opus-45
ernos models set venice/llama-3.3-70b
```

List all available models:

```bash
ernos models list | grep venice
```

## Configure via `ernos configure`

1. Run `ernos configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ErnOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ernos agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
ernos agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
ernos agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
ernos agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
ernos agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `ernos models list` to see currently available models. Some models may be temporarily offline.
