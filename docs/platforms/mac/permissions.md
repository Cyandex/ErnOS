...
- Same path: run the app from a fixed location (for ErnOS, `dist/ErnOS.app`).
...
```bash
sudo tccutil reset Accessibility ai.ernos.mac
sudo tccutil reset ScreenCapture ai.ernos.mac
...
Workaround: move files into the ErnOS workspace (`~/.ernos/workspace`) if you want to avoid per-folder grants.
