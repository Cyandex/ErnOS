...
service via the external `ernos` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.ernos.gateway`
  (or `ai.ernos.<profile>` when using `--profile`/`ERNOS_PROFILE`; legacy `com.ernos.*` is supported).
