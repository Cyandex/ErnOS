---
summary: "ErnOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ErnOS on a Raspberry Pi
  - Running ErnOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ErnOS on Raspberry Pi

## Goal

Run a persistent, always-on ErnOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ErnOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled ernos

# Enable if not
sudo systemctl enable ernos

# Start on boot
sudo systemctl start ernos
```
...
```bash
# Check logs
journalctl -u ernos --no-pager -n 100

# Common fix: rebuild
cd ~/ernos  # if using hackable install
npm run build
sudo systemctl restart ernos
```
