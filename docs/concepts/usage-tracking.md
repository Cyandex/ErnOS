...
- `/usage cost` in chats: local cost summary aggregated from ErnOS session logs.
- CLI: `ernos status --usage` prints a full per-provider breakdown.
- CLI: `ernos channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
