...
所有日志配置都在 `~/.ernos/ernos.json` 的 `logging` 下。
...
- ErnOS 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `ernos plugins enable diagnostics-otel` 启用插件。
...
- `ernos.tokens`（计数器，属性：`ernos.token`、`ernos.channel`、`ernos.provider`、`ernos.model`）
- `ernos.cost.usd`（计数器，属性：`ernos.channel`、`ernos.provider`、`ernos.model`）
- `ernos.run.duration_ms`（直方图，属性：`ernos.channel`、`ernos.provider`、`ernos.model`）
- `ernos.context.tokens`（直方图，属性：`ernos.context`、`ernos.channel`、`ernos.provider`、`ernos.model`）

消息流：

- `ernos.webhook.received`（计数器，属性：`ernos.channel`、`ernos.webhook`）
- `ernos.webhook.error`（计数器，属性：`ernos.channel`、`ernos.webhook`）
- `ernos.webhook.duration_ms`（直方图，属性：`ernos.channel`、`ernos.webhook`）
- `ernos.message.queued`（计数器，属性：`ernos.channel`、`ernos.source`）
- `ernos.message.processed`（计数器，属性：`ernos.channel`、`ernos.outcome`）
- `ernos.message.duration_ms`（直方图，属性：`ernos.channel`、`ernos.outcome`）

队列 + 会话：

- `ernos.queue.lane.enqueue`（计数器，属性：`ernos.lane`）
- `ernos.queue.lane.dequeue`（计数器，属性：`ernos.lane`）
- `ernos.queue.depth`（直方图，属性：`ernos.lane` 或 `ernos.channel=heartbeat`）
- `ernos.queue.wait_ms`（直方图，属性：`ernos.lane`）
- `ernos.session.state`（计数器，属性：`ernos.state`、`ernos.reason`）
- `ernos.session.stuck`（计数器，属性：`ernos.state`）
- `ernos.session.stuck_age_ms`（直方图，属性：`ernos.state`）
- `ernos.run.attempt`（计数器，属性：`ernos.attempt`）
...
- `ernos.webhook.error`
  - `ernos.channel`、`ernos.webhook`、`ernos.chatId`、`ernos.error`
- `ernos.message.processed`
  - `ernos.channel`、`ernos.outcome`、`ernos.chatId`、`ernos.messageId`、`ernos.sessionKey`、`ernos.sessionId`、`ernos.reason`
- `ernos.session.stuck`
  - `ernos.state`、`ernos.ageMs`、`ernos.queueDepth`、`ernos.sessionKey`、`ernos.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `ernos doctor`。
