...
ErnOS supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, ErnOS performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). ErnOS validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/ErnOS QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/ernos"],
...
When reload-time activation fails after a healthy state, ErnOS enters degraded secrets state.
...
```bash
ernos secrets audit --check
ernos secrets configure
ernos secrets audit --check
```
...
- plaintext values at rest (`ernos.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `ernos.json`
...
- `ernos secrets configure --providers-only`
- `ernos secrets configure --skip-provider-setup`
...
```bash
ernos secrets apply --from /tmp/ernos-secrets-plan.json
ernos secrets apply --from /tmp/ernos-secrets-plan.json --dry-run
```
...
ErnOS intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, ErnOS runtime no longer depends on plaintext `auth.json`.
