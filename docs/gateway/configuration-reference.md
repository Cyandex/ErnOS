...
description: "Complete field-by-field reference for ~/.ernos/ernos.json"
summary: "Complete reference for every ErnOS config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.ernos/ernos.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — ErnOS uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `ernos doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["ernos-dm"] },
      guilds: {
        "123456789012345678": {
...
- ErnOS additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `ernos plugins install @ernos/mattermost`.
...
ErnOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `ernos channels add` (or channel onboarding) while still on a single-account top-level channel config, ErnOS moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `ernos doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@ernos"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `ernos.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, ErnOS auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, ErnOS assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.ernos/sandboxes`
...
noVNC observer access uses VNC auth by default and ErnOS emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `ernos-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.ernos/workspace",
        agentDir: "~/.ernos/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@ernos"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, ErnOS starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:ernos`      | All built-in tools (excludes provider plugins)                                           |
...
ErnOS uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.ernos/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ERNOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `ernos onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `ernos onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `ernos onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `ernos onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `ernos onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `ernos onboard --auth-choice minimax-api`.
...
- Loaded from `~/.ernos/extensions`, `<workspace>/.ernos/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `ernos plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ERNOS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ERNOS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.ernos-dev` + port `19001`), `--profile <name>` (uses `~/.ernos-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.ernos/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-ernos-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ERNOS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ERNOS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__ernos__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__ernos__/a2ui/`
...
- Also serves A2UI at `/__ernos__/a2ui/`.
...
- Hostname defaults to `ernos`. Override with `ERNOS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.ernos/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `ernos dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.ernos/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ERNOS_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.ernos/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/ernos-vault-resolver",
...
- Legacy OAuth imports from `~/.ernos/credentials/oauth.json`.
...
- Default log file: `/tmp/ernos/ernos-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `ernos doctor --fix` can strip unknown keys).
