...
`ernos doctor` is the repair + migration tool for ErnOS. It fixes stale
...
- Extra workspace dir detection (`~/ernos`).
...
you to run `ernos doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.ernos/ernos.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.ernos/sessions/` to `~/.ernos/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.ernos/agent/` to `~/.ernos/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.ernos/credentials/*.json` (except `oauth.json`)
  - to `~/.ernos/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `ernos doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.ernos` folders exist across
  home directories or when `ERNOS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.ernos/ernos.json` is
...
offers to remove them and install the ErnOS service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named ErnOS gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `ernos doctor --generate-gateway-token` to force token
...
- `ernos doctor` prompts before rewriting supervisor config.
- `ernos doctor --yes` accepts the default repair prompts.
- `ernos doctor --repair` applies recommended fixes without prompts.
- `ernos doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `ernos gateway install --force`.
