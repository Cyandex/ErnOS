...
- During heartbeat runs, ErnOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ernos sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ErnOS skips the heartbeat run entirely (no model call).
...
```bash
ernos system event --text "Check for urgent follow-ups" --mode now
```
