...
read_when:
  - Running ErnOS behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ErnOS
...
- You run ErnOS behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. ErnOS checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. ErnOS extracts the user identity from the configured header
...
- ErnOS can stay on loopback HTTP behind the proxy.
...
If ErnOS itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://ernos.example.com
    to: http://ernos-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`ernos security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update ErnOS config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `ernos security audit` and review findings
