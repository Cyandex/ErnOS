...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ErnOS
...
Running ErnOS via Node with `tsx` fails at startup with:

```
[ernos] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ernos status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ernos.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ernos.mjs status` works on Node 25.
