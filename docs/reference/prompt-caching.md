...
- With Anthropic API-key auth profiles, ErnOS seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, ErnOS injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
ErnOS exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.ernos/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ERNOS_STATE_DIR/logs/cache-trace.jsonl`
...
- `ERNOS_CACHE_TRACE=1` enables cache tracing.
- `ERNOS_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ERNOS_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ERNOS_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ERNOS_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
