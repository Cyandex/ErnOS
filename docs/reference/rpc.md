...
ErnOS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ErnOS owns lifecycle when `channels.signal.autoStart=true`.
...
- ErnOS spawns `imsg rpc` as a child process (legacy iMessage integration).
