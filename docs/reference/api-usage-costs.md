...
ErnOS features that can generate provider usage or paid API calls.
...
- `ernos status --usage` and `ernos channels list` show provider **usage windows**
...
ErnOS can pick up credentials from:
...
`ernos models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
