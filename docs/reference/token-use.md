---
summary: "How ErnOS builds prompt context and reports token usage + costs"
read_when:
...
ErnOS tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

ErnOS assembles its own system prompt on every run. It includes:
...
For images, ErnOS downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from ErnOS session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `ernos status --usage` and `ernos channels list` show
...
`cacheWrite`. If pricing is missing, ErnOS shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. ErnOS can
...
Anthropic's 1M context window is currently beta-gated. ErnOS can inject the
...
ErnOS skips the `context-1m-*` beta header because Anthropic currently
