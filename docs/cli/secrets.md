---
summary: "CLI reference for `ernos secrets` (reload, audit, configure, apply)"
read_when:
...
Use `ernos secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
ernos secrets configure
ernos secrets configure --plan-out /tmp/ernos-secrets-plan.json
ernos secrets configure --apply --yes
ernos secrets configure --providers-only
ernos secrets configure --skip-provider-setup
ernos secrets configure --json
```
...
- `configure` targets secret-bearing fields in `ernos.json`.
...
ernos secrets apply --from /tmp/ernos-secrets-plan.json
ernos secrets apply --from /tmp/ernos-secrets-plan.json --dry-run
ernos secrets apply --from /tmp/ernos-secrets-plan.json --json
```
...
- `ernos.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.ernos/.env` known secret keys whose values were migrated
