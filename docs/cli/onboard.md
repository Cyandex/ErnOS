---
summary: "CLI reference for `ernos onboard` (interactive onboarding wizard)"
read_when:
...
```bash
ernos onboard
ernos onboard --flow quickstart
ernos onboard --flow manual
ernos onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `ernos dashboard` (Control UI, no channel setup).
