...
read_when:
  - You want to place an outbound voice call from ErnOS
...
Voice calls for ErnOS via a plugin. Supports outbound notifications and
...
- Use `ernos voicecall ...` or the `voice_call` tool
...
```bash
ernos plugins install ./extensions/voice-call
